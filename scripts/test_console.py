...
class Session:
    def __init__(self, scenario="balanced", width=80, height=18):
        ...
        self.until(lambda s: "Ready" in s)
...
def main():
    ...
    try:
        session.send(b".")
        session.until(lambda s: "Paused · 0m" in s)
        session.send(b"p/SIM-1\r\r")
        session.until(lambda s: "Fee paid" in s and "Delivered" in s)
        session.snapshot("payment-summary")
        session.send(b"e")
        session.until(lambda s: "Payment evidence" in s and "reserved @0" in s)
        session.snapshot("same-tick-reservation")
        session.send(b"\x1b")
        session.until(lambda s: "Fee paid" in s)
        session.send(b"\x1b")
        session.send(b"/\x7f\x7f\x7f\x7f\x7f\r1")
        session.send(b"+++")
        session.until(lambda s: "100 tick/s" in s)
        session.send(b" ")
        session.until(lambda s: "Running" in s and int(re.search(r"Running · (\d+)m", s)[1]) >= 20)
        session.send(b" ")
        session.until(lambda s: "Paused" in s)
        minute = re.search(r"Paused · (\d+)m", session.screen.text())[1]
        session.read(0.2)
        assert re.search(r"Paused · (\d+)m", session.screen.text())[1] == minute
        session.snapshot("balanced-watch")
        session.send(b"p\x1b[H\r")
        session.until(lambda s: "Fee paid" in s)
        session.send(b"e\x1b[F")
        ...
        session.send(b"1\r")
        session.until(lambda s: "Transit" in s and "RTP" in s)
        session.snapshot("rail-detail")
        session.send(b"p")
        session.until(lambda s: "/RTP" in s)
        session.send(b"\x1b")
        session.send(b"\t")
        session.until(lambda s: "Same demand" in s and "Fees" in s)
        session.snapshot("comparison")
        session.send(b"\r")
        session.until(lambda s: "Fee paid" in s)
        session.snapshot("matched-payment")
        session.send(b"\x1b")
        session.send(b"e")
        session.until(lambda s: "Totals" in s)
        session.send(b"\x1b")
        session.send(b"r")
        session.until(lambda s: "Ready" in s)
        session.send(b"n")
        session.until(lambda s: "Seed 43" in s)
        session.send(b"c")
        session.until(lambda s: "Scenario" in s and "More arrivals" in s)
        session.send(b"j\r")
        session.until(lambda s: "pressure" in s and "Ready" in s)
        session.resize(120, 32)
        session.until(lambda s: "Payment routing" in s)
        ...
        session.abort()
        raise
    print("PASS controls, watch, comparison, matched investigation, evidence, search, scenario selection, resize, q cleanup")
    for preset, exit_key in [("pressure", b"\x1b"), ("outage", b"\x03"), ("limited", b"q"), ("disruptions", b"q")]:
        session = Session(preset)
        try:
            session.send(b"." * 80)
            session.until(lambda s: "Paused · 79m" in s)
            session.snapshot(f"{preset}-watch")
            session.send(b"5")
            ...
            session.abort()
            raise
        print(f"PASS {preset}: 80 ticks, diagnostics, comparison, exit {exit_key!r}, terminal modes restored")
